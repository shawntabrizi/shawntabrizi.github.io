...
The [Substrate Feeless Token Factory](https://github.com/shawntabrizi/substrate-feeless-token-factory) (SFTF) removes the pains and costs of blockchain transaction fees from end users, by offloading those costs onto the token owners and community contributors who want to support a token.

SFTF is a blockchain level protocol, developed on top of the Substrate blockchain framework, which provides an alternative mechanism for transfer fees on tokens, while still preventing nominal attack vectors. Ultimately, each token built on the network is backed by a fund of the native blockchain currency. This fund is used to pay for the transfers of that token, on behalf of the user.

First, we developed an ERC20 compatible token factory as a Substrate Runtime Module. This allows any user to easily create new tokens which can be used on the blockchain, without any coding necessary.

Next, we extended the ERC20 protocol, exposing a function called `try_free_transfer`, which allows a user to make free transfers of these tokens.  At the time of creation, the user creating the tokens is given the option to deposit funds into the account in order to initially fund the token. The transfer fees are taken from the fund for that token, and users are limited on the number of times they can make a free transfer per time period, as determined by the token creator.
